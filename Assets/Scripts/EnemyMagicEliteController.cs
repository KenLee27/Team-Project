...
public class EnemyMagicEliteController : MonoBehaviour, Ienemy
{
    ...
    void SpawnMonster()
    {
        ...
        // 구울 소환
        GameObject leftGhoul = Instantiate(ghoulPrefab, leftPosition, Quaternion.identity);
        GameObject rightGhoul = Instantiate(ghoulPrefab, rightPosition, Quaternion.identity);

        // 구울 스크립트에서 플레이어 할당 및 소환 상태 설정
        EnemyController2 leftGhoulScript = leftGhoul.GetComponent<EnemyController2>();
        EnemyController2 rightGhoulScript = rightGhoul.GetComponent<EnemyController2>();
