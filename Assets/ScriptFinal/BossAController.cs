...
public class BossAController : MonoBehaviour, Ienemy
{
    ...
    public Transform mageBarrier;
    public Transform lsBloodBox;

    enum State
    {
        IDLE,                       //보스 메이지 : 기본 상태
        CHASE,                      //보스 메이지 : 플레이어 추격 패턴
        QUAKE,                      //보스 메이지 : 땅울림 스킬 패턴
        CHARGE_QUAKE,               //보스 메이지 : 2페이즈 개막 패턴
        SCRATCH,                    //보스 메이지 : 근접 타격 패턴
        ATTACK_JUMP_MAGIC,          //보스 메이지 : 1페이즈 종료 점프 내려찍기 패턴
        ATTACK_JUMP_QUAKE,          //보스 메이지 : 근접 강한 땅울림 패턴
        LITTLE_SKILL_ATTACK,        //보스 메이지 : 원거리 약공격 스킬 패턴
        BIG_SKILL_ATTACK,           //보스 메이지 : 원거리 강공격 스킬 패턴
        ATTACK,                     //보스 메이지 : 원거리 2연격 패턴
        DASH,                       //보스 메이지 : 플레이어와 거리 좁히기 패턴
        BACK,                       //보스 메이지 : 제자리 귀환 패턴
        CHECK,                      //보스 메이지 : 상황 견제 패턴
        STUN,                       //보스 메이지 : 경직 패턴
        BIG_STUN,                   //보스 메이지 : 대경직 패턴
        DIE,                        //보스 메이지 : 사망 패턴
        REBORN,                     //보스 메이지 : 부활 패턴 (2페이즈 전조)
        COMBO,                      //보스 메이지 : 2페이즈 원거리 2연격 공격 패턴
        SSCRATCH,                   //보스 메이지 : 2페이즈 근접 2연격 공격 패턴
        BACKSTEP                    //보스 메이지 : 근접 > 원거리 공격 방식 전환 패턴
    }

    State state;

    public AudioSource audioSource;         //AudioSource 컴포넌트
    public AudioClip[] footSound;           //보스 메이지 : 발소리
    public AudioClip[] stunSound;           //보스 메이지 : 경직소리
    public AudioClip[] smallCrashSound;     //보스 메이지 : 충격파 소리 (작음)
    public AudioClip[] bigCrashSound;       //보스 메이지 : 충격파 소리 (큼)
    public AudioClip[] smallMagicSound;     //보스 메이지 : 원거리 마법 소리 (작음)
    public AudioClip[] bigMagicSound;       //보스 메이지 : 원거리 마법 소리 (큼)
    public AudioClip[] attackSound;         //보스 메이지 : 근거리 공격 타격 소리
    public AudioClip[] flightSound;         //보스 메이지 : 비행 소리
    public AudioClip[] attackedSound;       //보스 메이지 : 피격 소리
    public AudioClip[] clearSound;          //보스 메이지 : 클리어 시 재생되는 소리
    ...
    void Awake()
    {
        ...
        {
            Destroy(gameObject);
            mageBarrier.gameObject.SetActive(false);
            lsBloodBox.gameObject.SetActive(true);
        }
    }
    ...
    IEnumerator IDLE()
    {
        ...
        if (distanceToPlayer <= detectingRange && player_1.GetComponent<SuperPlayerController>().isStand) //탐지 범위 안에 플레이어가 들어오면
        {
            ...
            GameManager.Instance.PlaySelectedBackgroundMusic(6);
    ...
    IEnumerator DASH() //보스 메이지 : 돌진 패턴
    {
        ...
        if (isHit && isBigStun)                 //플레이어에 의한 피격이 유효하며 대경직이 발생할 시
        {
            isAttack = false;                   //공격 패턴 차단
            ChangeState(State.BIG_STUN);        //대경직 패턴으로 전환
            yield break;                        // 코루틴 함수 종료
        }
        if (HP < 0)                             //현재 체력이 0보다 작을 때
        {
            ChangeState(State.DIE);             //사망 패턴으로 전환
            yield break;                        //코루틴 함수 종료
        }
        if (distanceToPlayer <= attackRange)    //플레이어와의 거리가 공격 사거리보다 작거나 같을 때
        {
            ChangeState(State.SCRATCH);         //돌진 패턴 이후 근거리 공격 패턴으로 전환
        }
    ...
    IEnumerator DIE()
    {
        ...
        mageBarrier.gameObject.SetActive(false);
        lsBloodBox.gameObject.SetActive(true);
        PlayerPrefs.SetFloat("maxHPposion", 5);
        PlayerPrefs.SetFloat("maxMPposion", 5);
        PlayerPrefs.Save();
    }
    ...
    public void PRSStun()
    {
        int randomIndex = UnityEngine.Random.RandomRange(0, stunSound.Length);
        AudioClip selectedClip = stunSound[randomIndex];
        audioSource.PlayOneShot(selectedClip);
    }

    public void PRSSmallCrash()
    {
        int randomIndex = UnityEngine.Random.RandomRange(0, smallCrashSound.Length);
        AudioClip selectedClip = smallCrashSound[randomIndex];
        audioSource.PlayOneShot(selectedClip);
    }

    public void PRSBigCrash()
    {
        int randomIndex = UnityEngine.Random.RandomRange(0, bigCrashSound.Length);
        AudioClip selectedClip = bigCrashSound[randomIndex];
        audioSource.PlayOneShot(selectedClip);
    }

    public void PRSSmallMagic()
    {
        int randomIndex = UnityEngine.Random.RandomRange(0, smallMagicSound.Length);
        AudioClip selectedClip = smallMagicSound[randomIndex];
        audioSource.PlayOneShot(selectedClip);
    }

    public void PRSBigMagic()
    {
        int randomIndex = UnityEngine.Random.RandomRange(0, bigMagicSound.Length);
        AudioClip selectedClip = bigMagicSound[randomIndex];
        audioSource.PlayOneShot(selectedClip);
    }
    ...
    public void PRSFlight()
    {
        int randomIndex = UnityEngine.Random.RandomRange(0, flightSound.Length);
        AudioClip selectedClip = flightSound[randomIndex];
        audioSource.PlayOneShot(selectedClip);
    }
