...
public class EnemyController2 : MonoBehaviour, Ienemy
{
    ...
    public Slider hpSlider;                     // 몬스터 HP 슬라이더
    ...
    public void PlayRandomRoarSound()
    {
        if (roarSound.Length == 0)
        {
            Debug.LogWarning("No roar sounds assigned!");
            return;
        }

        int randomIndex = UnityEngine.Random.RandomRange(0, roarSound.Length);
        AudioClip selectedClip = roarSound[randomIndex];
        audioSource.PlayOneShot(selectedClip);
    }
