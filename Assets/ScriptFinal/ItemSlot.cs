...
public class ItemSlot : MonoBehaviour, IPointerClickHandler
{
    ...
    private void Start()
    {
        ...
        if (!isFull && defaultWeaponPrefab != null)
        {
            ...
            string itemDescription = "종류 : 한손 장검\r\n공격배수 : 1.25\r\n\r\n칼리오스 병사들이 주로 사용하던 무기이다."; // 해당 무기 설명으로 변경
            equipmentManager.AddItem(itemName, itemSprite, itemDescription);  // 슬롯에 기본 아이템 추가
        }
    ...
    //아이템 추가 (아이템 이름, 아이템 이미지, 아이템 설명)
    {
        this.itemName = itemName;                   //아이템 이름 설정
        this.itemSprite = itemSprite;               //아이템 이미지 설정
        this.itemDescription = itemDescription;     //아이템 설명 설정
        isFull = true;
