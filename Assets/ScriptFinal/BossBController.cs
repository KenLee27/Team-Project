...
public class BossBController : MonoBehaviour, Ienemy
{
    ...
    public Transform kamonBarrier;
    public Transform aRotBox;
    ...
    public AudioClip[] landCrashSound;
    ...
    void Awake()
    {
        ...
        if (PlayerPrefs.GetInt("Kamon", 0) == 1)
        {
            ...
            aRotBox.gameObject.SetActive(true);
            Destroy(gameObject);
        }
    }
    ...
    IEnumerator DIE()
    {
        ...
        castleDoor.gameObject.SetActive(false);
        kamonBarrier.gameObject.SetActive(false);
        aRotBox.gameObject.SetActive(true);

        if (PlayerPrefs.GetInt("Mage", 0) == 0)
        {
            PlayerPrefs.SetFloat("maxHPposion", 4);
            PlayerPrefs.SetFloat("maxMPposion", 4);
            PlayerPrefs.Save();
        }
    ...
    IEnumerator IDLE()
    {
        ...
        if (distanceToPlayer <= detectingRange) //탐지 범위 안에 플레이어가 들어오면
        {
            ...
            kamonBarrier.gameObject.SetActive(true);
            GameManager.Instance.VolumeUp();
            GameManager.Instance.PlaySelectedBackgroundMusic(5);
    ...
    public void PRSLandCrash()
    {
        int randomIndex = UnityEngine.Random.RandomRange(0, landCrashSound.Length);
        AudioClip selectedClip = landCrashSound[randomIndex];
        audioSource.PlayOneShot(selectedClip);
    }

    public void PRSCrash()
    {
    ...
    public void PRSCrawl()
    {
        int randomIndex = UnityEngine.Random.RandomRange(0, crawlSound.Length);
        AudioClip selectedClip = crawlSound[randomIndex];
        audioSource.PlayOneShot(selectedClip);
    }
